...
function ActivitiesList({ activities }) {
  ...
  async function handleApprove({ id }) {
    const response = await fetch("https://woodvale-test-2.onrender.com/activity/approve/" + id, {
      method: "POST",
      headers: {
  ...
  async function handleReject({ id }) {
    const response = await fetch("https://woodvale-test-2.onrender.com/activity/disapprove/" + id, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: "Bearer " + token,
